...
import Xutil
from config_camaras import ConfigCamaras
...
class GridWindow(QMainWindow):
    ...
    def crear_ui(self):
        ...
        for fila in range(self.filas):
            for col in range(self.columnas):
                ...
                if index < len(self.camaras_activas_config):
                    cell = Camara( id = self.camaras_activas_config[index].get("id"),
                               user = self.camaras_activas_config[index].get("user"),
                               password = self.camaras_activas_config[index].get("password"),
                               ip = self.camaras_activas_config[index].get("ip"),
                               port = self.camaras_activas_config[index].get("port"))
    ...
    def cargar_camaras(self):
        ...
        '''self.camaras_config = [
            ...
            {"mac": "CC-BA-BD-22-C0-4D", "ip": "", "id": "CEPY07", "user": "cepy2026",
             "password": "Castelar2026", "port": 554},
            {"mac": "CC-BA-BD-22-BA-65", "ip": "", "id": "CEPY08", "user": "cepy2026",
             "password": "Castelar2026", "port": 554}
        ]'''

        config_camaras = ConfigCamaras()
        config_camaras.cargar_camaras_config()
        self.camaras_activas_config = config_camaras.camaras
    ...
    def calcular_filas_columnas(self):
        numero_camaras = len(self.camaras_activas_config)
